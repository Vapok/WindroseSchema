...
namespace Windrose {
    WindroseEnumLoader::WindroseEnumLoader() : WindroseModLoaderBase("enums") {
        SetDisplayName(TEXT("Enum Loader"));
    }

    WindroseEnumLoader::~WindroseEnumLoader() {}

    void WindroseEnumLoader::OnLoad(const std::filesystem::path& loaderPath, const RC::StringType& modName, const EEngineLifecyclePhase& engineLifecyclePhase)
    {
    ...
    bool WindroseEnumLoader::CanInitialize(const EEngineLifecyclePhase& engineLifecyclePhase)
    {
        if (engineLifecyclePhase == EEngineLifecyclePhase::PostEngineInit)
        {
            return true;
    ...
    bool WindroseEnumLoader::OnInitialize()
    {
    ...
    void WindroseEnumLoader::LoadEnums(const nlohmann::json& data)
    {
    ...
    RC::Unreal::UEnum* WindroseEnumLoader::GetEnumByName(const RC::StringType& Name)
    {
