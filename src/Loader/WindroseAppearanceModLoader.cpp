...
namespace Windrose {
	WindroseAppearanceModLoader::WindroseAppearanceModLoader() : WindroseModLoaderBase("appearance")
	{
        SetDisplayName(TEXT("Appearance Mod Loader"));
	}

	WindroseAppearanceModLoader::~WindroseAppearanceModLoader()
	{
	}

    void WindroseAppearanceModLoader::OnLoad(const std::filesystem::path& loaderPath, const RC::StringType& modName, const EEngineLifecyclePhase& engineLifecyclePhase)
    {
    ...
    void WindroseAppearanceModLoader::OnAutoReload(const std::filesystem::path::string_type& modName, const std::filesystem::path& modFilePath)
    {
    ...
    bool WindroseAppearanceModLoader::CanInitialize(const EEngineLifecyclePhase& engineLifecyclePhase)
    {
        if (engineLifecyclePhase == EEngineLifecyclePhase::GameInstanceInit)
        {
            return true;
    ...
    bool WindroseAppearanceModLoader::OnInitialize()
    {
        try
        {
    ...
    void WindroseAppearanceModLoader::LoadAppearances(const nlohmann::json& data)
    {
	...
	void WindroseAppearanceModLoader::Add(const RC::Unreal::FName& RowId, RC::Unreal::UDataTable* DataTable, const nlohmann::json& Data, const std::vector<std::string>& RequiredFields)
	{
	...
	void WindroseAppearanceModLoader::AddColorPreset(const RC::Unreal::FName& ColorPresetId, const nlohmann::json& Data)
	{
	...
	void WindroseAppearanceModLoader::AddEquipment(const RC::Unreal::FName& EquipmentId, const nlohmann::json& Data)
	{
