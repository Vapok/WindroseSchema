...
namespace Windrose {
	WindroseRawTableLoader::WindroseRawTableLoader() : WindroseModLoaderBase("raw") {
        SetDisplayName(TEXT("Raw Table Loader"));
    }

	WindroseRawTableLoader::~WindroseRawTableLoader() {}

    void WindroseRawTableLoader::Apply(const RC::StringType& tableName, RC::Unreal::UDataTable* datatable)
    {
    ...
    void WindroseRawTableLoader::Apply(UECustom::UCompositeDataTable* compositeDatatable)
    {
    ...
    void WindroseRawTableLoader::Apply(const nlohmann::json& data, RC::Unreal::UDataTable* datatable, LoadResult& outResult)
    {
        for (auto& [dataKey, dataRow] : data.items())
        {
            if (dataKey == "Rows")
            {
                ...
                PS::Log<LogLevel::Error>(STR("When copying entries from FModel, make sure to not include the 'Rows' field and instead add your row entries directly. See https://okaetsu.github.io/WindroseSchema/docs/guides/rawtables/intro for more info.\n"));
                continue;
            }
    ...
    void WindroseRawTableLoader::OnLoad(const std::filesystem::path& loaderPath, const RC::StringType& modName, const EEngineLifecyclePhase& engineLifecyclePhase)
    {
    ...
    void WindroseRawTableLoader::OnAutoReload(const RC::StringType& modName, const std::filesystem::path& modFilePath)
    {
    ...
    bool WindroseRawTableLoader::CanInitialize(const EEngineLifecyclePhase& engineLifecyclePhase)
    {
        if (engineLifecyclePhase == EEngineLifecyclePhase::PostEngineInit)
        {
            return true;
    ...
    bool WindroseRawTableLoader::OnInitialize()
    {
        return true;
    }

    void WindroseRawTableLoader::OnDatatableSerialized(RC::Unreal::UDataTable* datatable)
    {
        if (!datatable) return;
    ...
    void WindroseRawTableLoader::HandleFilters(RC::Unreal::UDataTable* datatable, const nlohmann::json& data, LoadResult& outResult)
    {
        try
        {
            if (data.is_null())
    ...
    void WindroseRawTableLoader::AddRow(RC::Unreal::UDataTable* datatable, const FName& rowName, const nlohmann::json& data, LoadResult& outResult)
    {
    ...
    void WindroseRawTableLoader::EditRow(RC::Unreal::UDataTable* datatable, const FName& rowName, uint8* row, const nlohmann::json& data, LoadResult& outResult)
    {
        try
        {
    ...
    void WindroseRawTableLoader::DeleteRow(RC::Unreal::UDataTable* datatable, const RC::Unreal::FName& rowName, LoadResult& outResult)
    {
        datatable->RemoveRow(rowName);
        outResult.SuccessfulDeletions++;
    }

    void WindroseRawTableLoader::ModifyRowProperties(RC::Unreal::UDataTable* datatable, const FName& rowName, void* rowPtr, const nlohmann::json& data,
    ...
    void WindroseRawTableLoader::AddToTableDataMap(const std::string& datatableName, const nlohmann::json& data)
    {
