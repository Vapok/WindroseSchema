...
namespace Windrose {
    WindroseSpawnLoader::WindroseSpawnLoader() : WindroseModLoaderBase("spawns") {
        SetDisplayName(TEXT("Custom Spawn Loader"));
    }

    WindroseSpawnLoader::~WindroseSpawnLoader() {
    ...
    void WindroseSpawnLoader::Reload(const std::filesystem::path::string_type& modName, const nlohmann::json& data)
    {
        UnloadMod(modName);
    ...
    void WindroseSpawnLoader::OnLoad(const std::filesystem::path& loaderPath, const RC::StringType& modName, const EEngineLifecyclePhase& engineLifecyclePhase)
    {
    ...
    void WindroseSpawnLoader::OnAutoReload(const std::filesystem::path::string_type& modName, const std::filesystem::path& modFilePath)
    {
    ...
    bool WindroseSpawnLoader::CanInitialize(const EEngineLifecyclePhase& engineLifecyclePhase)
    {
        if (engineLifecyclePhase == EEngineLifecyclePhase::GameInstanceInit)
        {
            return true;
    ...
    bool WindroseSpawnLoader::OnInitialize()
    {
        try
        {
            m_bossSpawnerLocationData = GetDatatableByName("DT_BossSpawnerLoactionData");

            auto CleanupWorld_FuncPtr = Windrose::SignatureManager::GetSignature("UWorld::CleanupWorld");
    ...
    void WindroseSpawnLoader::OnCellLoaded(UECustom::UWorldPartitionRuntimeLevelStreamingCell* cell)
    {
    ...
    void WindroseSpawnLoader::OnCellUnloaded(UECustom::UWorldPartitionRuntimeLevelStreamingCell* cell)
    {
        DestroySpawnersInCell(cell);
        m_loadedCells.Remove(cell);
    }

    void WindroseSpawnLoader::SetupWorldPartitionHooks()
    {
    ...
    void WindroseSpawnLoader::LoadSpawns(const RC::StringType& modName, const nlohmann::json& data)
    {
        if (!data.is_array())
        {
    ...
    void WindroseSpawnLoader::RegisterSpawn(const std::filesystem::path::string_type& modName, const nlohmann::json& value)
    {
    ...
    void WindroseSpawnLoader::RegisterSheet(const std::filesystem::path::string_type& modName, PS::SpawnerInfo& spawnerInfo, const nlohmann::json& value)
    {
        ...
        {
            static auto ENUM_EWindroseSpawnedCharacterType = UECustom::UObjectGlobals::StaticFindObject<UEnum*>(nullptr, nullptr,
                STR("/Script/Windrose.EWindroseSpawnedCharacterType"));

            std::string spawnerType;
            PS::JsonHelpers::ParseString(value, "SpawnerType", spawnerType);
            spawnerInfo.SpawnerType = static_cast<RC::Unreal::uint8>(
                PS::EnumHelpers::GetEnumValueByName(ENUM_EWindroseSpawnedCharacterType, spawnerType));
    ...
    void WindroseSpawnLoader::RegisterMonoNPC(const std::filesystem::path::string_type& modName, PS::SpawnerInfo& spawnerInfo, const nlohmann::json& value)
    {
        ...
        // Optional: creature summoned when the NPC is attacked.
    ...
    void WindroseSpawnLoader::ProcessCellSpawners(UECustom::UWorldPartitionRuntimeLevelStreamingCell* cell)
    {
    ...
    void WindroseSpawnLoader::CreateSpawner(UECustom::UWorldPartitionRuntimeLevelStreamingCell* cell, PS::SpawnerInfo& spawnerInfo)
    {
    ...
    void WindroseSpawnLoader::SpawnMonoNPC(UECustom::UWorldPartitionRuntimeLevelStreamingCell* cell, PS::SpawnerInfo& spawnerInfo)
    {
        static auto bpClass = UECustom::UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Game/Windrose/Blueprint/Spawner/BP_MonoNPCSpawner.BP_MonoNPCSpawner_C"));
        if (!bpClass)
        {
    ...
    void WindroseSpawnLoader::SpawnSheet(UECustom::UWorldPartitionRuntimeLevelStreamingCell* cell, PS::SpawnerInfo& spawnerInfo)
    {
        static auto sheetBPClass = UECustom::UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Game/Windrose/Blueprint/Spawner/BP_WindroseSpawner_Standard.BP_WindroseSpawner_Standard_C"));
        if (!sheetBPClass)
        {
            PS::Log<LogLevel::Error>(STR("Unable to get class BP_WindroseSpawner_Standard, failed to spawn sheet\n"));
            return;
        }
        ...
        auto palSheet = static_cast<AWindroseSpawnerStandard*>(spawnedActor);

        WindroseSpawnerGroup spawnerGroup;
        for (auto& spawnGroupListItem : spawnerInfo.SpawnGroupList)
        {
            ...
            for (auto& listEntry : spawnGroupListItem.WindroseList)
            {
                spawnerGroup.WindroseList.push_back(listEntry);
            }
        }
    ...
    void WindroseSpawnLoader::AddBossSpawnLocationToMap(PS::SpawnerInfo& spawnerInfo)
    {
        ...
        if (firstGroup.WindroseList.size() == 0)
        {
            PS::Log<LogLevel::Error>(STR("Failed to add boss icon to map, WindroseList has no entries.\n"));
            return;
        }

        auto& firstEntry = firstGroup.WindroseList.at(0);

        auto characterId = firstEntry.WindroseId;
        if (characterId == NAME_None)
        {
            characterId = firstEntry.NPCID;
        }
        ...
        newRow->SetValue<int>(STR("Level"), firstEntry.Level);

        spawnerInfo.bHasMapIcon = true;
    }

    void WindroseSpawnLoader::RemoveBossSpawnLocationFromMap(PS::SpawnerInfo& spawnerInfo)
    {
        if (!m_bossSpawnerLocationData) {
    ...
    void WindroseSpawnLoader::DestroySpawnersInCell(UECustom::UWorldPartitionRuntimeLevelStreamingCell* cell)
    {
        for (auto& spawn : m_spawns)
        {
            if (spawn.Cell == cell)
    ...
    void WindroseSpawnLoader::UnloadMod(const std::filesystem::path::string_type& modName)
    {
    ...
    void WindroseSpawnLoader::CleanupSpawns()
    {
    ...
    void WindroseSpawnLoader::OnWorldCleanup(RC::Unreal::UWorld* thisWorld, bool bSessionEnded, bool bCleanupResources, RC::Unreal::UWorld* newWorld)
    {
