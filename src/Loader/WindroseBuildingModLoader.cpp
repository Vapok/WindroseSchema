...
namespace Windrose {
	WindroseBuildingModLoader::WindroseBuildingModLoader() : WindroseModLoaderBase("buildings") {
        SetDisplayName(TEXT("Building Mod Loader"));
    }

	WindroseBuildingModLoader::~WindroseBuildingModLoader() {}

    void WindroseBuildingModLoader::OnLoad(const std::filesystem::path& loaderPath, const RC::StringType& modName, const EEngineLifecyclePhase& engineLifecyclePhase)
    {
    ...
    void WindroseBuildingModLoader::OnAutoReload(const RC::StringType& modName, const std::filesystem::path& modFilePath)
    {
    ...
    bool WindroseBuildingModLoader::CanInitialize(const EEngineLifecyclePhase& engineLifecyclePhase)
    {
        if (engineLifecyclePhase == EEngineLifecyclePhase::GameInstanceInit)
        {
            return true;
    ...
    bool WindroseBuildingModLoader::OnInitialize()
    {
        try
        {
    ...
    void WindroseBuildingModLoader::LoadBuildings(const nlohmann::json& data)
    {
	...
	void WindroseBuildingModLoader::Add(const RC::Unreal::FName& BuildingId, const nlohmann::json& Data)
	{
	...
	void WindroseBuildingModLoader::Edit(uint8_t* ExistingRow, const RC::Unreal::FName& BuildingId, const nlohmann::json& Data)
	{

	}

	void WindroseBuildingModLoader::SetupBuildData(const RC::Unreal::FName& BuildingId, const nlohmann::json& Data)
	{
        ...
        else
        {
            ...
            {
                PS::Log<LogLevel::Warning>(STR("When adding new buildings, including 'RedialIndex' will not do anything as this will be handled by WindroseSchema to avoid collisions. This is a warning and will not affect the loading of your building '{}'.\n"), BuildingId.ToString());
            }
            ...
            try
            {
                for (FProperty* Property : TFieldRange<FProperty>(TableRowStruct, EFieldIterationFlags::IncludeSuper))
                {
                    ...
                    // Let WindroseSchema handle Radial Index to avoid collisions with the build wheel in-game.
                    if (PropertyName == "RedialIndex")
                    {
	...
	void WindroseBuildingModLoader::SetupIconData(const RC::Unreal::FName& BuildingId, const nlohmann::json& Data)
	{
        if (Data.contains("IconTexture"))
        {
	...
	void WindroseBuildingModLoader::SetupAssignments(const RC::Unreal::FName& BuildingId, const nlohmann::json& Data)
	{
        if (!Data.is_array())
        {
    ...
    void WindroseBuildingModLoader::SetupAssignment(const RC::Unreal::FName& BuildingId, const nlohmann::json& Data)
    {
	...
	void WindroseBuildingModLoader::SetupCropData(const RC::Unreal::FName& BuildingId, const nlohmann::json& Data)
	{
	...
	void WindroseBuildingModLoader::SetupItemProductData(const RC::Unreal::FName& BuildingId, const nlohmann::json& Data)
	{
        ImportJson(BuildingId, Data, m_mapObjectItemProductDataTable);
	}

    void WindroseBuildingModLoader::SetupTechnologyData(const RC::Unreal::FName& BuildingId, const nlohmann::json& Data)
    {
    ...
    void WindroseBuildingModLoader::SetupTranslations(const RC::Unreal::FName& BuildingId, const nlohmann::json& Data)
    {
    ...
    void WindroseBuildingModLoader::SetupTranslation(const RC::StringType& RowKey, RC::Unreal::UDataTable* DataTable, const nlohmann::json& Value)
    {
    ...
    void WindroseBuildingModLoader::ImportJson(const RC::Unreal::FName& BuildingId, const nlohmann::json& Data, RC::Unreal::UDataTable* DataTable)
    {
    ...
    RC::StringType WindroseBuildingModLoader::GetAssignIDSuffixByWorkType(const std::string& WorkType)
    {
        if (WorkType == "EWindroseWorkType::Seeding" || WorkType == "Seeding")
        {
            return STR("_5");
        }
        else if (WorkType == "EWindroseWorkType::Watering_Farm" || WorkType == "Watering_Farm")
        {
            return STR("_2");
        }
        else if (WorkType == "EWindroseWorkType::FarmHarvest" || WorkType == "FarmHarvest")
        {
            return STR("_4");
        }
        else
    ...
    int WindroseBuildingModLoader::GetNextRadialIndex()
    {
        return ++RadialIndex;
    }
}